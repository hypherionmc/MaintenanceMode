...
public class MaintenanceModeCommand {
    ...
    public static int listAllowedUsers(CommandSource stack) {
        ...
        if (names.length == 0) {
            ...
            stack.sendSuccess(new TranslationTextComponent("There are %s allowed player(s): %s", names.length, String.join(", ", names)), false);
        }
    ...
    private static int setMotd(CommandSource stack, String message) {
        MaintenanceModeConfig config = CommonClass.config;
        if (config == null) {
            config = new MaintenanceModeConfig();
        }
